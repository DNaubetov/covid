...
tickFont = {'size':12, 'color':"rgb(30,30,30)", 'family':"Courier новые_,, monospace"}
# сортировка данных
...
# обнавление данных
def refreshData():
    allData = loadData("time_series_covid19_confirmed_global.csv", "старые_Заболевшие") \
        .merge(loadData("time_series_covid19_deaths_global.csv", "старые_Умершие")) \
        .merge(loadData("time_series_covid19_recovered_global.csv", "старые_Выздоровевшие"))
...
app.layout = html.Div(
    style={ 'font-family':"Courier новые_, monospace" },
    children=(
        html.H1('Ковид-19'),
        html.Div(className="row", children=[
            ...
            html.Div(className="four columns", children=[
                html.H5('Выберите фильтр'),
                dcc.Checklist(
                    id='metrics',
                    options=[{'label': m, 'value': m} for m in ["Заболевшие", "Умершие", "Выздоровевшие"]],
                    value=['Заболевшие'])
            ])
        ]),
        dcc.Graph(
...
def filtered_data(country, state):
    ...
    newCases.columns = [column.replace('старые_', 'новые_') for column in newCases.columns]
...
def barchart(data, metrics, prefix="", yaxisTitle=""):
    figure = go.Figure(data=[
        go.Bar(
            ...
            marker_color={ 'Умершие':'rgb(200,30,30)', 'Выздоровевшие':'rgb(30,200,30)', 'Заболевшие':'rgb(100,140,240)'}[metric]
...
def update_plots(country, state, metrics, n):
    ...
    barchart_new = barchart(data, metrics, prefix="новые_", yaxisTitle="Статистика за день")
    barchart_cum = barchart(data, metrics, prefix="старые_", yaxisTitle="Статситика с начала заражения")
